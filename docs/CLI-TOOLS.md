...
Load all context needed for a specific workflow in one call. Returns JSON with project info, config, state, and workflow-specific data. `init onboard [--fast]` reports brownfield signals, planning-doc candidates, codebase-map completeness, fast-map readiness, partial planning state, and onboarding summary status for `/gsd-onboard`.
